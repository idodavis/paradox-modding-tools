...
export const currentPage = writable<"hub" | "settings" | "compare-tool" | "modding-docs" | "merge-tool" | "inventory">(
  "hub",
);
export const lastPage = writable<"hub" | "settings" | "compare-tool" | "modding-docs" | "merge-tool" | "inventory">(
  "hub",
);
export const appSettings = writable<Record<string, string>>({});

export const gameInstallPath = derived([game, appSettings], ([$g, $s]) =>
  $g === "CK3" ? ($s["ck3.install_path"] ?? "") : ($s["eu5.install_path"] ?? ""),
);
